...
void ACryptRaiderCharacter::MoveForward(float Value)
{
	if (Value != 0.0f)
	{
		...
		if (FootstepSound && GetCharacterMovement()->IsMovingOnGround() && CanPlayFootstepSound())
		{
...
bool ACryptRaiderCharacter::CanPlayFootstepSound() const
{
	// Calculate the time since the last footstep sound was played
	...
	return TimeSinceLastFootstep >= FootstepCooldown;
}